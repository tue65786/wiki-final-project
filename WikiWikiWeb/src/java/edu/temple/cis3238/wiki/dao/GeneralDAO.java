...
public class GeneralDAO implements IGeneralDAO {
   ...
   @Override
   public boolean assignOnlyTopicsTagNamesFromTagCSV(TopicVO _topicVO, String _tagNameCSV) {
	  CallableStatement cs = null;
	  int rowsAffected = 0;
	  if (_topicVO == null || _tagNameCSV == null || _tagNameCSV.isEmpty()) {
		 LOG.logp(Level.INFO, this.getClass().getName(),
				 "assignOnlyTopicsTagNamesFromTagCSV(TopicVO,String _tagNameCSV)",
				 "Invalid params. Topic and TagsCSV must not be null.");
		 return false;
	  }

	  try {
		 cs = dbc.getConn().prepareCall(DB_STRINGS.TOPIC_ASSIGN_ONLY_TAG_TAG_NAME_CSV);
		 _tagNameCSV = StringUtils.stripInvalidChars(_tagNameCSV);
		 cs.setInt(1, _topicVO.getTopicID());
		 cs.setString(2, _tagNameCSV);
		 ...
		 LOG.log(Level.SEVERE, null, ex);
		 System.err.println(DbUtils.printJDBCExceptionMsg(ex));
	  } catch (Exception ex) {
		 LOG.log(Level.SEVERE, null, ex);
	  }
	  return false;
   ...
   private boolean changeTopicTagsBinding(TopicVO _topicVO, ArrayList<TagsVO> _tagsVO, boolean assign) {
	  ...
	  if (_topicVO == null || _tagsVO == null || _tagsVO.isEmpty()) {
		 LOG.logp(Level.INFO, this.getClass().getName(),
				 "changeTopicTagsBinding(TopicVO,TagsVO[],boolean)",
				 "Invalid params. Topic and Tags Array must not be null.");
		 return false;
	  }
	  try {
   ...
   public boolean deleteTag(TagsVO _vo) {
	  ...
	  if (_vo == null || _vo.getTagID() <= 0) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "deleteTag(TagsVO)",
				 "Invalid params. TagsVO not be null and tagID must be a positive integer.");
	  }
	  try {
   ...
   public boolean deleteTopic(TopicVO _vo) {
	  ...
	  if (_vo == null || _vo.getTopicID() <= 0) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "deleteTopic(int)",
				 "Invalid params. TopicVO must not be null and TopicID must be a positive integer.");
		 return false;
	  }
	  try {
   ...
   public boolean deleteUser(UsersVO _vo) {
	  ...
	  if (_vo == null || _vo.getUserID() <= 0) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "deleteUser(int)",
				 "Invalid params. UsersVO can not be null and UserID must be a positive integer.");
		 return false;
	  }
	  try {
   ...
   public UsersVO findUserByName(String _username) {
	  ...
	  if (StringUtils.toS(_username).isEmpty()) {
		 LOG.logp(Level.INFO, this.getClass().getName(), "findUserByUserName(String)",
				 "Invalid params");
		 return null;
	  }
	  try {
		 ...
		 if (rs.next()) {
			...
			LOG.logp(Level.INFO, this.getClass().getName(),
					"findUserByUserNameAndPassword(String, String)", "User not found");
		 }
   ...
   public UsersVO findUserByUserNameAndPassword(String _username, String _password) {
	  ...
	  if (_username.length() * _password.length() == 0) {
		 LOG.logp(Level.INFO, this.getClass().getName(),
				 "findUserByUserNameAndPassword(String, String)",
				 "Invalid params. Username and Password can not be null");
		 return null;
	  }
	  try {
   ...
   private TagsVO getTag(int _id, String _name) {
	  ...
	  if (StringUtils.toS(_name).isEmpty() && _id <= 0) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "getTag(int, String)",
				 "Invalid params. Either TagID or TagName must be set.");
		 return null;
	  }
	  try {
   ...
   public ArrayList<TagsVO> getTagsByTopicID(int _topicid) {
	  ...
	  TagsVO vo = null;
	  if (_topicid <= 0) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "getTagsByTopicID(int)",
				 "Invalid params. TopicID must be a positive int.");
		 return null;
	  }
	  try {
   ...
   public TopicVO getTopicByID(int _id) {
	  if (_id <= 0) {
		 LOG.logp(Level.INFO, this.getClass().getName(), "getTopic(int)",
				 "Invalid param. Specified _id is not a valid TopicID");
		 return null;
	  }
   ...
   public TopicVO getTopicByName(String _name) {
	  ...
	  if (StringUtils.toS(_name).isEmpty()) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "getTopic(String)",
				 "Invalid param. Invalid _name, TopicName can not be null");
		 return null;
	  }
	  if (voList != null && !voList.isEmpty()) {
		 return voList.get(0);
   ...
   public ArrayList<TopicHistoryVO> getTopicHistoryByTopicID(int _topicId) {
	  ...
	  if (_topicId <= 0) {
		 LOG.logp(Level.INFO, this.getClass().getName(), "getTopicHistoryByTopicID(int)",
				 "Invalid param. Specified _id is not a valid TopicID");
		 return null;
	  }
	  try {
   ...
   public ArrayList<TopicHistoryVO> getTopicHistoryByTopicName(String _topicName) {
	  ...
	  if (StringUtils.toS(_topicName).isEmpty()) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "getTopicHistoryByTopicName(String)",
				 "Invalid param. TopicName can not be null.");
		 return null;
	  }
	  try {
   ...
   public ArrayList<TopicVO> getTopicsByTagID(int _id) {
	  ...
	  ArrayList<TopicVO> voList = new ArrayList<TopicVO>();
	  if (_id <= 0) {
		 LOG.logp(Level.INFO, this.getClass().getName(), "getTopicsByTagID(int)",
				 "Invalid params. TagID must be positive int.");
		 return null;
	  }
	  try {
   ...
   public ArrayList<TopicVO> getTopicsByTagName(String _name) {
	  ...
	  if (tag == null || tag.getTagID() <= 0) {
		 LOG.logp(Level.INFO, this.getClass().getName(), "getTopicsByTagName(String)",
				 "Invalid tag name");
		 return voList;
	  }
	  try {
...
@Override
   ...
   public ArrayList<TopicVO> searchTopic(String _query, boolean loadCollections,String sortColumn, int maxResults) {
	  ...
	  if (StringUtils.toS(_query).isEmpty()) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "searchTopic(String)",
				 "Invalid params. Query can not be null.");
		 return null;
	  }
	  try {
   ...
   public boolean updateTopic(TopicVO _vo) {
	  ...
	  int rowsAffected = 0;
	  if (_vo == null || _vo.getTopicID() <= 0) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "updateTopic(TopicVO)",
				 "Invalid param. TopicVO id null or TopicID is not valid.");
		 return false;
   ...
   public boolean updateUser(UsersVO _vo) {
	  ...
	  int rowsAffected = 0;
	  if (_vo == null || _vo.getUserID() <= 0) {
		 LOG.logp(Level.WARNING, this.getClass().getName(), "updateUser(UsersVO)",
				 "Invalid param. UserVO is null or UserID is not valid.");
		 return false;
	  }
	  try {
